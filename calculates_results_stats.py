"""
Calculate stats against results.
...
def calculates_results_stats(results_dic):
    """
    Calculate statistics from the results of the program.

    Parameters:
      results_dic - Dictionary with key as image filename and value List
             (index)idx 0 = pet image label (string)
                    ...
                    idx 4 = 1/0 (int)  where 1 = Classifier classifies image
                            ...
                            'as-NOT-a' dog.
    Returns:
     results_stats_dic - Dictionary contains the results statistics (either
    ...
    a: Number of Correct Dog matches
        Both labels dogs: results_dic[key][3] = 1 and results_dic[key][4] = 1
    b: Number of Dog Images
        Pet Label is a dog: results_dic[key][3] = 1
    c: Number of Correct (NOT SPECIES) Non-Dog matches
        Both lbls NOT of dogs:
        results_dic[key][3] = 0 and results_dic[key][4] = 0
    ...
    if not d:
        ...
        results_stats_dic.update({'pct_id_other_species': 0})
    else:
    ...
    results_stats_dic.update({'n_correct_notdogs': c})
    if not b:
    ...
    if not z:
        results_stats_dic.update({'pct_label_match': 0})
    else:
